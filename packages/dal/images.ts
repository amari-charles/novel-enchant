...
export async function nextAttempt(sceneId:string){
  const { data, error } = await supa().from("images").select("attempt").eq("scene_id", sceneId);
...
export async function createImageAttempt(sceneId:string, attempt:number, payload:any){
  const { data, error } = await supa().from("images").insert({ scene_id: sceneId, attempt, ...payload }).select("id").single();
  if (error) throw error; return data.id as string;
}
export async function finalizeImage(imageId:string, patch:any){
  const { error } = await supa().from("images").update({ ...patch, status:'completed' }).eq("id", imageId);
  if (error) throw error;
}